...
public class HospitalInfoActivity extends AppCompatActivity {
    ...
    private void handleAddToFavBtnClicked() {
        ...
        AtomicBoolean exist = new AtomicBoolean(sharedPreferences.getString(hospitalId, null) != null);

        if (exist.get()) {
            addToMyFavBtn.setText("從我最愛移除");
            addToMyFavBtn.setBackground(getDrawable(R.drawable.favorite_sbtn2));
        } else {
            addToMyFavBtn.setText("加入我的最愛");
            addToMyFavBtn.setBackground(getDrawable(R.drawable.favorite_sbtn));
        }
